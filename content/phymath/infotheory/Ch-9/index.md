...
>- **典型列编号**： 根据渐近等分性，极大概率出现的典型列（$0$ 出现约 $90$ 次，$1$ 出现约 $10$ 次）总数约为 $2^{nH(\mathcal{V})}$。对于本例，典型序列大约有 $2^{100 \times 0.469} \approx 2^{47}$ 条。如果只对这部分高概率序列进行编号，只需约 $47 \text{ bits}$，此时编码速率 $R_s \approx 0.47$ 
>
>该信源的信源熵 $H(\mathcal{V})=-0.9\log0.9-0.1\log0.1\approx0.469$。可见通过典型列编码可以降低信源编码速率，降低对信道容量 $C$ 的要求。