...
\begin{align}
I(x;y)
&=\sum_x\sum_yp(x,y)\log\frac{p(x\vert y)}{q(x)}\\
&=\sum_x\sum_yp(x,y)\log\frac{p(x,y)}{q(x)\cdot\omega(y)}\\
&\geq-\sum_x\sum_yp(x,y)\left\{\frac{q(x)\omega(y)}{p(x,y)}-1\right\}\\
&=0
\end{align}
...
$$
>[!Note] **相对熵的应用**
>假设数据通过未知分布 $p(x)$ 生成，我们想要对 $p(x)$ 进行建模，即使用参数分布 $p(x\vert\theta)$ 来近似该分布：
