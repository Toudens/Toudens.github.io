...
> 5. 若 $0<\lambda<1$ ，定义 $\beta_i=\frac{\theta_i}{\lambda},(i=1,2,\cdots,k)$，则 $\beta_i\geq 0$，$\sum_{i=1}^k\beta_i=1$ ，有：$$f(\sum_{i=1}^{k+1}\theta_i\alpha_i)=f(\lambda\sum_{i=1}^k\beta_i\alpha_i+\theta_{k+1}\alpha_{k+1})\geq\lambda f(\sum_{i=1}^k\beta_i\alpha_i)+\theta_{k+1}f(\alpha_{k+1})$$ 
>对 $k$ 个点应用归纳假设：$$f(\sum_{i=1}^k\beta_i\alpha_i)\geq\sum_{i=1}^k\beta_if(\alpha_i)$$ 
>代入上式得：
