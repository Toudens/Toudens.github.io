...
>
>**注：** 本人认为此处的解释不甚完美，在事件的自信息的解释中不将 $I(x_k)$ 视为不确定性的表现，此处却引入了不确定性。由于 $x_k$ 和 $y_j$ 的位置互换不影响互信息的值，不妨观察上述定义式，对于等式第二行的中间结果可以得到：$I(x_k;y_j)=I(x_k)+I(y_j)-I(x_k,y_j)$ 。即事件 $\{X=x_k\}$ 与 $\{Y=y_j\}$ 的自信息之和与二者的联合自信息（见下文）之差，因此互信息应解释为：两个信息同时发生时，是产生了新信息（负）还是产生了冗余信息（正），或解释为两个信息同时发生时产生冗余信息的程度。
...
$$
>**事件的联合自信息的本质：**
>事件 $\{X=x_k\}$ 和 $\{Y=y_i\}$ 同时发生需要的信息量，或同时发生后对外界提供的信息量。
